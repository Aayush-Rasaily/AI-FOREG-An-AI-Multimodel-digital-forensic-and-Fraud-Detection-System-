"""Deepfake video detector plugin."""
...
class DeepfakeVideoDetector(VideoAIDetector):
    """Face-swap and deepfake detection with pluggable model backends."""
    ...
    def metadata(self) -> VideoDetectorMetadata:
        return VideoDetectorMetadata(
            ...
            description=(
                "Detects face-swap and deepfake manipulation in sampled video frames."
            ),
    ...
    async def predict(self, context: VideoAnalysisContext) -> VideoDetectorOutput:
        ...
        if not self._model_available:
            finding = unavailable_finding(
                detector=self.name,
                category=VideoFindingCategory.DEEPFAKE,
