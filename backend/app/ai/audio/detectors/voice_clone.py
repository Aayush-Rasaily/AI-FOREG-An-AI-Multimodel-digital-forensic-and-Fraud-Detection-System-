"""Voice cloning indicator detector plugin."""
...
class VoiceCloneDetector(AudioAIDetector):
    """Detect voice cloning indicators when a model is configured."""
    ...
    def load(self, *, device: str) -> None:
        ...
        self._loaded = True
        self._model_available = False
        if not self._settings.voice_clone_model_enabled:
            return
        path_value = self._settings.voice_clone_model_path
        if not path_value:
            return
        path = Path(path_value)
        if not path.exists():
            return
        verify_model_hash(path, self._settings.voice_clone_model_sha256)
    ...
    def metadata(self) -> AudioDetectorMetadata:
        return AudioDetectorMetadata(
            ...
            description="Detects voice cloning indicators.",
            supported_tasks=("voice_clone_detection",),
            model_name="audio_voice_clone",
            model_version=self._settings.voice_clone_model_version,
            framework="PYTORCH",
    ...
    async def predict(self, context: AudioAnalysisContext) -> AudioDetectorOutput:
        ...
        if not self._model_available:
            finding = unavailable_finding(
                detector=self.name,
                category=AudioFindingCategory.VOICE_CLONE,
