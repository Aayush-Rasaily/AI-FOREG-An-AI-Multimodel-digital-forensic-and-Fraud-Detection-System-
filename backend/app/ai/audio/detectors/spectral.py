...
class SpectralDetector(AudioAIDetector):
    name = "spectral"
    version = "1.0.0"
    ...
    def metadata(self) -> AudioDetectorMetadata:
        return AudioDetectorMetadata(
            ...
            description="Detects spectral discontinuities across temporal windows.",
            supported_tasks=("spectral_analysis",),
            model_name="spectral_classical",
    ...
    def _analyze(
        ...
        for previous, current in zip(metrics, metrics[1:], strict=False):
            delta = abs(float(current["centroid"]) - float(previous["centroid"]))
            if delta < 400:
                continue
            start_ms = int(current["start_time_ms"])
            end_ms = int(current["end_time_ms"])
            temporal = temporal_from_ms(start_ms, end_ms, "SPECTRAL_DISCONTINUITY")
            finding = AudioAIFindingItem(
                detector=self.name,
                category=AudioFindingCategory.SPECTRAL,
                severity=Severity.LOW,
                description="Spectral centroid shift detected.",
                explanation=(
                    "Spectral characteristics differ from the preceding segment. "
                    "This is an observational indicator, not proof of fraud."
                ),
                method=DetectionMethod.CLASSICAL,
                confidence=min(0.7, delta / 2000.0),
                temporal=temporal,
                metadata={"centroid_delta_hz": delta},
                model_name="spectral_classical",
                model_version=self.version,
                model_framework="NATIVE",
                limitations="Scene or speaker changes can produce similar shifts.",
            )
