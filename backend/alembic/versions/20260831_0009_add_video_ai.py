"""Add video AI forensic analysis tables.
...
def upgrade() -> None:
    """Create video AI forensic analysis persistence tables."""
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            sa.Enum(
                ...
                "UNAVAILABLE",
                "CANCELLED",
                name="video_analysis_run_status",
                native_enum=False,
                length=16,
            ),
            nullable=False,
    ...
    op.create_index(
        "ix_video_analysis_runs_evidence_id",
        "video_analysis_runs",
        ["evidence_id"],
    )
    op.create_table(
        ...
        sa.Column(
            ...
            sa.Enum(
                ...
                "HIGH",
                "CRITICAL",
                name="video_ai_finding_severity",
                native_enum=False,
                length=16,
            ),
            nullable=False,
        ...
        sa.Column("end_timestamp_ms", sa.Integer(), nullable=True),
        sa.Column("artifact_id", sa.Uuid(as_uuid=True), nullable=True),
        sa.Column("metadata", sa.JSON(), nullable=False),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False),
        sa.ForeignKeyConstraint(
            ["analysis_run_id"],
            ["video_analysis_runs.id"],
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(["evidence_id"], ["evidence.id"], ondelete="CASCADE"),
    ...
    op.create_index(
        "ix_video_ai_findings_analysis_run_id",
        "video_ai_findings",
        ["analysis_run_id"],
    )
    op.create_index(
        "ix_video_ai_findings_evidence_id",
        "video_ai_findings",
        ["evidence_id"],
    )
    op.create_index(
        "ix_video_ai_findings_detector",
        "video_ai_findings",
        ["detector"],
    )
    op.create_table(
        "video_ai_finding_regions",
        sa.Column("id", sa.Uuid(as_uuid=True), nullable=False),
        ...
        sa.ForeignKeyConstraint(
            ["finding_id"],
            ["video_ai_findings.id"],
            ondelete="CASCADE",
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(
        "ix_video_ai_finding_regions_finding_id",
        "video_ai_finding_regions",
        ["finding_id"],
    )
...
def downgrade() -> None:
    """Drop video AI forensic analysis persistence tables."""

    op.drop_index(
        "ix_video_ai_finding_regions_finding_id",
        table_name="video_ai_finding_regions",
    )
    op.drop_table("video_ai_finding_regions")
    op.drop_index("ix_video_ai_findings_detector", table_name="video_ai_findings")
    op.drop_index("ix_video_ai_findings_evidence_id", table_name="video_ai_findings")
    op.drop_index(
        "ix_video_ai_findings_analysis_run_id",
        table_name="video_ai_findings",
    )
    op.drop_table("video_ai_findings")
    op.drop_index(
        "ix_video_analysis_runs_evidence_id",
        table_name="video_analysis_runs",
    )
